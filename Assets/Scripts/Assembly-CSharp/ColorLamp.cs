...
public class ColorLamp : MonoBehaviour
{
	...
	private void Start()
	{
		...
		m_fCount = 0f;
		m_fCurTime = Random.Range(randomtime.x, randomtime.y);
		m_State = State.Wait;
	}
	...
	private void Update()
	{
		...
		case State.Wait:
			if (!(m_fCount < m_fCurTime))
			{
				...
				m_fCurSpeed = Random.Range(alphaspeed.x, alphaspeed.y);
				m_nTimes = (int)Random.Range(timesrange.x, timesrange.y);
