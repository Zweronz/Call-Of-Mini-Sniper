	...
	public override void UpdateGameing(float deltaTime)
	{
		...
		else
		{
			if (Screen.lockCursor)
			{
				...
				if (!m_bAim)
				{
					m_CameraScript.YawCamera(Input.GetAxisRaw("Mouse X") * deltaTime);
					m_CameraScript.PitchCamera(Input.GetAxisRaw("Mouse Y") * deltaTime);
				}
				else
				{
					m_CameraScript.AimMove(Input.GetAxisRaw("Mouse X") * 4f, Input.GetAxisRaw("Mouse Y") * 2f);
				}
			}
		}
	}
