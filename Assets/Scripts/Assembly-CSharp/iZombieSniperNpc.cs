...
public class iZombieSniperNpc
{
	...
	public class MoveState : NpcState
	{
		...
		public Vector3 GetNextPointPath()
		{
			if (m_Path.Count > 0)
			{
				...
				if (m_bRandPos)
				{
					result.x += Random.Range(-2f, 2f);
					result.z += Random.Range(-2f, 2f);
				}
				m_Path.RemoveAt(0);
				return result;
			}
	...
	public Vector3 OnHitRandom()
	{
		if (m_bFrameOrBone)
		{
			...
			{
				int num = Random.Range(0, m_MeshFilter.mesh.vertexCount);
		...
		{
			int num2 = Random.Range(0, m_SkinnedMeshRenderer.sharedMesh.vertexCount);
	...
	public void PlayAnimDirectlyRandom(string sAnim, float speed = 1f, bool bLoop = false)
	{
		...
		{
			m_Model.GetComponent<Animation>()[sAnim].time = Random.Range(0f, m_Model.GetComponent<Animation>()[sAnim].length);
			m_Model.GetComponent<Animation>()[sAnim].speed = speed;
			m_Model.GetComponent<Animation>()[sAnim].wrapMode = ((!bLoop) ? WrapMode.Once : WrapMode.Loop);
