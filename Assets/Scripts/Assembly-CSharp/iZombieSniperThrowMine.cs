...
public class iZombieSniperThrowMine : MonoBehaviour
{
	...
	public void Initialize(Vector3 v3Dir, float speed, float width, float height)
	{
		...
		m_v3Dir = v3Dir;
		m_fSpeed = speed;
		m_fBoomTime = Random.Range(9f, 12f);
		...
		if (m_Rigidbody != null)
		{
			...
			m_Rigidbody.AddTorque(base.transform.forward * Random.Range(-10f, 10f));
			m_Rigidbody.AddTorque(base.transform.up * Random.Range(-10f, 10f));
		}
		m_bActive = true;
		m_bGround = false;
	}
